...
FROM (
 SELECT
  ...
  CASE
    -- APP 老版本房源单页
    WHEN request_uri RLIKE '^/mobile/member/inventories/[0-9]+'
      THEN regexp_extract(request_uri,'^/mobile/member/inventories/([0-9]+)',1)
    -- 新版 推广单页
 ...
 WHERE (
   request_uri RLIKE '^/mobile/member/inventories/[0-9]+'
   OR request_uri RLIKE '^/mobile/member/inventories/[0-9]+/[0-9]+'
   OR request_uri RLIKE '^/mobile/member/inventory/detail/[0-9]+/[0-9]+'
 )
 -- AND p_dt = '2016-05-10'
 AND p_dt BETWEEN DATE_SUB(${dealDate},14) AND ${dealDate}
