...
from 
dw_db.dw_inventory_detail_info_daily m
left join (
...
inner join dw_temp_angejia.xiaohei_a_community b on m.community_id=b.community_id
where m.i_status='2' and m.city_id<>'3'
and to_date(m.i_created_at)<=${dealDate}
group by
 b.team_name
;
export hive dw_temp_angejia.xiaohei_followup_temp
to mysql dw_temp_angejia.xiaohei_followup_info
;
...
from
dw_db.dw_inventory_detail_info_daily m inner join dw_temp_angejia.xiaohei_a_community b on m.community_id=b.community_id
where m.i_status='2' and m.city_id<>'3' and to_date(m.i_created_at)=${dealDate}
;


export hive dw_temp_angejia.xiaohei_community_qualityup
to mysql dw_temp_angejia.xiaohei_community_qualityup
;
...
) m left join dw_db.dw_broker_summary_basis_info_daily c on m.user_id=c.user_id and c.p_dt=${dealDate}
group by c.agent_name
;

export hive dw_db_temp.dw_inventory_verification_stat_rpt
to mysql dw_temp_angejia.dw_inventory_verification_stat_rpt
;
...
group by b.community_id,m.stat_date 
;
...
,nvl(a.baopanshu,0) as yg_cnt
,nvl(b.gj_cnt,0) as gj_cnt
,nvl(c.agent_cnt,0) as agent_cnt
from dw_temp_angejia.xiaohei_a_community a 
inner join dw_db_temp.a_community_qualityup_daily_temp b on a.community_id=b.community_id
left join dw_db_temp.a_community_qualityup_daily_agent_temp c on trim(a.team_name) =trim(c.agent_name)
;

export hive dw_db_temp.a_community_qualityup_daily
to mysql dw_temp_angejia.a_community_qualityup_daily;
