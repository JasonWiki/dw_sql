...
-- 获取最近 7 天的运行日志
DROP TABLE IF EXISTS dw_db_temp.jason_monitor_report_log;
CREATE TABLE IF NOT EXISTS dw_db_temp.jason_monitor_report_log AS
SELECT
  s_1.id,
  s_1.report_id,
  s_1.status,
...
WHERE
  -- 今天开始算，7天前
  date(started) >= date_sub(${dealDate}, interval 7 day)
AND
  -- 到今天
  date(started) <= ${dealDate}
;

-- 获取 Minireport 当天运行详细数据
DROP TABLE IF EXISTS dw_db_temp.jason_monitor_report_daily;
CREATE TABLE IF NOT EXISTS dw_db_temp.jason_monitor_report_daily AS
SELECT
  t_2.id AS user_id,
  t_2.user_name,
  t_1.id AS report_id,
  ...
  (SELECT
      diff_second
    FROM
      dw_db_temp.jason_monitor_report_log
    WHERE
      report_id = t_1.id
    AND
      -- 成功的运行记录
...
LEFT JOIN (
  ...
  FROM
    dw_db_temp.jason_monitor_report_log AS s_2
  WHERE
    -- 成功运行的 job
    s_2.status = 2
  GROUP BY
...
-- 把当天的数据保存起来
CREATE TABLE IF NOT EXISTS dw_service.minireport_job_sd like dw_db_temp.jason_monitor_report_daily;
DELETE FROM dw_service.minireport_job_sd WHERE p_dt = ${dealDate};
INSERT INTO dw_service.minireport_job_sd SELECT * FROM dw_db_temp.jason_monitor_report_daily;
